    ...
    <x-card class="p-10 max-w-lg mx-auto mt-24">
        ...
        <form action="{{ route('listing.create') }}" method="POST" enctype="">
            ...
            <div class="mb-6">
                ...
                <input type="text" class="border border-gray-200 rounded p-2 w-full" name="tags" placeholder="Example: Laravel, Backend, Postgres, etc" value="{{old('location')}}"/>
                @error('tags')
                    <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                <button class="bg-laravel text-white rounded py-2 px-4 hover:bg-black">
                    <a href="{{route('listing.manage')}}" class="text-black ml-4"> 
                        Create Gig
                    </a>
                </button>
